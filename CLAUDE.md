...
- Theme is light by default (`defaultTheme: "light"`) via `next-themes`; the `Providers` wrapper in `app/providers.tsx` is a client component that wraps `NextThemesProvider`. Dark mode uses the `.dark` class strategy (`@custom-variant dark (&:is(.dark *))`).
